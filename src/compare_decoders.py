...
class DecodersComparison:
    def __init__(self, cp1, cp2, pca_dims, split_ratio, subsample_subgroups, subsample_subgroups_index, subsample_list,
                 step):
        ...
        self.subsample_subgroups = subsample_subgroups
        self.subsample_subgroups_index = subsample_subgroups_index
    ...
    def get_cca_decoder_score(self, sub_x, sub_y):
        try:
            ...
            check_pca_dims = min(len(temp_cca.data['cp2']['proc_x']), len(temp_cca.data['cp1']['proc_x']), self.pca_dims)
    ...
    def get_regression_fit_score(self, temp_cca, temp_cca_transformer, sub_x, sub_y):
        try:
            ...
            wpost, _ = ridge_fit(b0=self.day0_decoder_scale, x_format=temp_x_format, y_format=temp_y_format, my_alpha=100.0)
...
def process_subsample_data(cp1_original, cp2_original, pca_dims, split_ratio, index_group, subsample_subgroups,
                           subsample_subgroups_index, subsample_list, step):
    ...
    day0_decoder, day0_transformer, day0_decoder_no_offset, offset, day0_decoder_scale = get_day0_decoder(cp1=cp1,
                                                                                                          pca_dims=pca_dims)
    ...
    for i in range(len(subsample_list)):
        ...
        for idx, k in enumerate(np.arange(0, percent_data - step_sample, step_sample)):
            ...
            # Get PCA decoder score
            vaf_score, error_message = get_pca_decoder_score(cp2_raw=cp2_raw, cp2_test_raw=cp2_test_raw,
                                                             pca_dims=pca_dims, sub_x=sub_x, sub_y=sub_y, test_y=test_y)
            ...
            # Get CCA decoder score
            try:
                vaf_score, error_message, temp_cca, temp_cca_transformer = get_cca_decoder_score(cp1_raw=cp1,
                                                                                                 cp2_raw=cp2_raw,
                                                                                                 cp2_test_raw=cp2_test_raw,
                                                                                                 day0_decoder=day0_decoder,
                                                                                                 day0_transformer=day0_transformer,
                                                                                                 pca_dims=pca_dims,
                                                                                                 sub_x=sub_x,
                                                                                                 sub_y=sub_y)
                if vaf_score is not None:
            ...
            # Regression fit
            try:
                vaf_score, error_message = get_regression_fit_score(cp2_raw=cp2_raw, cp2_test_raw=cp2_test_raw,
                                                                    temp_cca=temp_cca,
                                                                    temp_cca_transformer=temp_cca_transformer,
                                                                    day0_decoder_scale=day0_decoder_scale,
                                                                    pca_dims=pca_dims, sub_x=sub_x, sub_y=sub_y)
                if vaf_score is not None:
...
def compare_decoders_all_perms(cp1, cp2, subsample_list, multiThread=False, multiProcess=False, num_processes=6,
                               step=0.005, pca_dims=8, split_ratio=0.8):
    """
    ...
    if multiThread is True:
        ...
        futures = [executor.submit(process_subsample_data, cp1_raw, cp2_raw, pca_dims, split_ratio, index_group,
                                   subsample_subgroups, subsample_subgroups_index, subsample_list, step) for index_group
        ...
        for index_group in range(num_processes):
            result = process_subsample_data(cp1_raw, cp2_raw, pca_dims, split_ratio, index_group, subsample_subgroups,
                                            subsample_subgroups_index, subsample_list, step)
