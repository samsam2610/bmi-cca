...
def vaf(x, xhat, round_values=True):
    ...
    if round_values is True:
        return np.round((1 - (np.sum(np.square(x - xhat)) / np.sum(np.square(x)))), 2)
    else:
...
# def parameter_fit(x, y, c):
#     """
#     05/01/2023 - Test Least Square function from numpy as alternative to the old one
#     """
#     x_plus_bias = np.c_[np.ones((np.size(x, 0), 1)), x]
#     H = np.linalg.lstsq(x_plus_bias, y, rcond=1)
#     # H = sp_lstsq(x_plus_bias, y, overwrite_a=True, overwrite_b=True,lapack_driver='gelsy', check_finite=False)
#     return H[0]
